...
Pages use `BaseLayout.astro` for the shared HTML shell (meta tags, fonts, global styles). The home page is three components: `Hero.astro` (logo + name + "Software studio · building in crypto & AI"), `Project.astro` (project rows: ailiste → ailiste.app, troniti → troniti.com, plainai → plainai.tech), `Footer.astro`.
...
- Accent: TRON-red (`#c23631`); PlainAI brand blue `#93cafa` is used for its card border, the "AI" word in the tagline, and the "ai" part of its logo; ailiste purple `#a978d0` for its row — the mid class of ailiste's choropleth ramp, because the product's own `#5f2b91` was chosen against a white map and vanishes on this one
- Font: Inter
